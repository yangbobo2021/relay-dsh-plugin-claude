...
import {
  CLAUDE_IMPORT_PATH,
  createClaudeImportHandler,
  registerClaudeImportRoute,
} from "../claude-import-route.js";

test("Claude import registers one exact DSH Web route", () => {
  let registration = null;
  const dispose = () => {};
  const result = registerClaudeImportRoute({
    webServer: {
      register(value) {
        registration = value;
        return dispose;
  ...
  assert.equal(registration.path, CLAUDE_IMPORT_PATH);
  assert.equal(typeof registration.handler, "function");
});

test("scan returns eligible Session identities for only the selected registered Workspace", async () => {
  const service = importer();
  const handler = createClaudeImportHandler({ importer: service, workspaceRegistry: registry() });
...
test("import streams monotonic progress and a final aggregate result", async () => {
  ...
  await handler(request({
    body: { action: "import", cwd: "/workspace/relay", sessionIds: ["session-ready"] },
  }), response);
  ...
  assert.deepEqual(service.imports, [{ cwd: "/workspace/relay", sessionIds: ["session-ready"] }]);
});

test("import keeps the omitted sessionIds contract for compatible clients", async () => {
  const service = importer();
  const handler = createClaudeImportHandler({ importer: service, workspaceRegistry: registry() });
  const response = responseRecorder();
  ...
  assert.equal(response.status, 200);
  assert.deepEqual(service.imports, [{ cwd: "/workspace/relay", sessionIds: undefined }]);
});

test("route rejects unsafe methods, bodies, Workspaces, and remote callers", async () => {
  const service = importer();
  const handler = createClaudeImportHandler({
    importer: service,
    workspaceRegistry: registry(),
    token: "import-secret",
    maxBodyBytes: 128,
  ...
  await handler(request({
    body: { action: "import", cwd: "/workspace/relay", sessionIds: ["same", "same"] },
  }), duplicateIds);
  ...
  return {
    ...
    async scanWorkspace(cwd) {
      ...
      return {
        ...
        entries: [
          {
            session: { sessionId: "session-ready", customTitle: "Ready title", summary: "ignored", cwd, lastModified: 30 },
            status: "ready",
          },
          {
            session: { sessionId: "session-recoverable", customTitle: null, summary: "Recoverable\npreview", cwd, lastModified: 20 },
            status: "recoverable",
          },
          {
            session: { sessionId: "session-existing", customTitle: "Bound", summary: "bound", cwd, lastModified: 10 },
            status: "existing",
          },
        ],
      };
    ...
    async importWorkspace(cwd, { sessionIds, onProgress }) {
      this.imports.push({ cwd, sessionIds });
