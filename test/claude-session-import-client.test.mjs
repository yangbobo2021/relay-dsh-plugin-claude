...
import {
  ...
  scanClaudeWorkspace,
} from "../src/client/claude-session-import-client.mjs";
...
test("scan uses the aggregate endpoint and surfaces server failures", async () => {
  ...
  const result = await scanClaudeWorkspace("/workspace/relay", fetchOk);
  assert.equal(result.summary.ready, 1);
  assert.deepEqual(JSON.parse(requests[0].init.body), { action: "scan", cwd: "/workspace/relay" });

  await assert.rejects(
    scanClaudeWorkspace("/workspace/missing", async () => new Response(
...
test("import sends selected Session IDs, forwards progress, and requires one complete frame", async () => {
  const updates = [];
  let importBody = null;
  const complete = await importClaudeWorkspace("/workspace/relay", {
  ...
  await assert.rejects(
    importClaudeWorkspace("/workspace/relay", {}, async () => new Response(
  ...
  await assert.rejects(
    importClaudeWorkspace("/workspace/relay", {}, async () => new Response(
      stream(['{"type":"error","message":"Claude SDK unavailable"}\n']),
      { status: 200 },
    )),
