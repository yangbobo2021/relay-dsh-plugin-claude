	...
	factory: (require) => {
		...
		function installModelSelection(ctx, preset, provider, otherProvider) {
			...
			const sync = () => {
				...
				(async () => {
					...
					const target = selectedPreset === preset ? models.groups.find((group) => group.id === provider) : models.current.provider === provider ? models.groups.find((group) => group.id !== provider && group.id !== otherProvider) : void 0;
		...
		async function apply(ctx) {
			...
			const unsubscribe = withConversationRuntime(ctx, (inner) => {
				...
				return installModelSelection(inner, "relay-claude", "relay-claude", "relay-codex");
			});
			return async () => {
				unsubscribe();
			};
