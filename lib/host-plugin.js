...
function extractFinalAnswerImagePaths(text) {
	...
	collectMatches(matches, visible, new RegExp("[\"']([^\"'\\r\\n]+" + IMAGE_SUFFIX + ")[\"']", "gi"), 1);
	collectMatches(matches, visible, new RegExp("(?:^|[\\s(\\[])((?:\\.{0,2}/|/)[^\\s<>\"'`]+?" + IMAGE_SUFFIX + ")(?=$|[\\s)\\],.;:!?])", "gi"), 1);
...
async function promoteFinalAnswerImages({ text, cwd, attachments, structuredImages = [], structuredImageData = [], signal }) {
...
async function snapshotWorkspaceImage(path, cwd, attachments, signal) {
	...
	if (!mediaType) throw outputImageError("the image type is unsupported", "CLAUDE_IMAGE_OUTPUT_TYPE_UNSUPPORTED");
